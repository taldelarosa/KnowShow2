...
public class ConfigurationService : IConfigurationService
{
    ...
    public async Task<ConfigurationResult> LoadConfiguration()
    {
        ...
        try
        {
            ...
            {
                _logger.LogWarning("Configuration file not found, generating default configuration - Operation: {OperationId}, Path: {ConfigPath}",
                    operationId, _configFilePath);
                
                var createResult = await CreateDefaultConfigurationFile();
                if (!createResult.Success)
                {
                    _logger.LogError("Failed to create default configuration file - Operation: {OperationId}, Path: {ConfigPath}, Error: {Error}, Duration: {Duration}ms",
                        operationId, _configFilePath, createResult.ErrorMessage, stopwatch.ElapsedMilliseconds);
                    return ConfigurationResult.Failure($"Configuration file not found and failed to create default: {createResult.ErrorMessage}");
                }
                
                _logger.LogInformation("Default configuration file created successfully - Operation: {OperationId}, Path: {ConfigPath}",
                    operationId, _configFilePath);
            }
    ...
    /// <summary>
    /// Creates a default configuration file with sensible default values.
    /// </summary>
    /// <returns>Result indicating success or failure of file creation.</returns>
    private async Task<(bool Success, string? ErrorMessage)> CreateDefaultConfigurationFile()
    {
        try
        {
            ...
            var defaultConfig = new
            {
                ...
                renameConfidenceThreshold = 0.6m,
                ...
                bulkProcessing = new
                {
                    ...
                    defaultMaxErrorsBeforeAbort = (int?)null,
                    defaultFileProcessingTimeout = "00:05:00",
                    defaultFileExtensions = new[] { ".mkv", ".mp4", ".avi" },
                    maxBatchSize = 10000,
                    maxConcurrency = 32,
        ...
        {
            _logger.LogError(ex, "Failed to create default configuration file at: {ConfigPath}", _configFilePath);
