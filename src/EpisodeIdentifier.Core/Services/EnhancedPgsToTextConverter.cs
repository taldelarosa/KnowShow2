...
public class EnhancedPgsToTextConverter
{
    ...
    private readonly bool _useOcrFallback;
    ...
    public async Task<string> ConvertPgsToText(byte[] pgsData, string language = "eng")
    {
        ...
        if (!_useOcrFallback)
        {
            _logger.LogError("pgsrip is not available and OCR fallback is disabled. Use --use-ocr-fallback flag to enable fallback conversion.");
            throw new InvalidOperationException("PGS conversion failed: pgsrip unavailable and OCR fallback disabled. Enable with --use-ocr-fallback flag.");
        }
    ...
    /// <summary>
    /// Check if PGS conversion is available (either pgsrip or fallback, if enabled)
    /// </summary>
    public async Task<bool> IsConversionAvailableAsync()
    {
        // pgsrip is always preferred if available
        if (await _pgsRipService.IsAvailableAsync())
        {
            return true;
        }

        // Only check fallback if it's enabled
        if (_useOcrFallback)
        {
            return _fallbackConverter.IsOcrAvailable();
        }
    ...
    public bool IsOcrAvailable()
    {
        ...
        return qualityInfo.PgsRipAvailable || (qualityInfo.FallbackAvailable && _useOcrFallback);
    }
}
