...
public class VideoFileProcessingService : IVideoFileProcessingService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Step 1: Validate video file format
            ...
            if (!videoExtensions.Contains(fileExtension))
            {
                result.Error = new IdentificationError
                {
                    ...
                    Message = $"The provided file format '{fileExtension}' is not supported. Supported formats: {string.Join(", ", videoExtensions)}"
                };
            ...
            if (!subtitleTracks.Any())
            {
                result.Error = new IdentificationError
                {
                    ...
                    Message = "No PGS or text subtitles could be found in the video file"
                };
                result.ProcessingCompleted = DateTime.UtcNow;
                return result;
            }

            // Step 3: Check if OCR tools are available for PGS processing
            var pgsTracks = subtitleTracks.Where(t =>
                t.CodecName == "hdmv_pgs_subtitle" ||
                t.CodecName == "dvd_subtitle").ToList();

            if (pgsTracks.Any() && !_pgsConverter.IsOcrAvailable())
            {
                result.Error = new IdentificationError
                {
